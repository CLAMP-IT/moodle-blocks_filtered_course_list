...
class config_validator extends \block_filtered_course_list\local\config_validator{
    ...
    public $names = array('expanded', 'courselistheading', 'catlistheading');
     ...
     */
    public function validate_courselistheading($value) {
        if (empty($value)) {
            $value = get_string('courses');
     ...
     * @return string|boolean the validated value, or false if the value is not valid
     */
    public function validate_catlistheading($value) {
        if (empty($value)) {
            $value = get_string('categories');
            $value = strip_tags($value);
        }
        return $value;
    }
